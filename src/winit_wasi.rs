...
use wasi_surface_wasmtime::{Surface, SurfaceDesc, SurfaceProxy};
...
pub struct WinitEventToSurfaceProxy {
    pointer_pos: (f64, f64),
    modifiers: ModifiersState,
    proxy: SurfaceProxy,
}

impl WinitEventToSurfaceProxy {
    pub fn new(proxy: SurfaceProxy) -> Self {
        Self {
            pointer_pos: (0.0, 0.0),
    ...
    pub fn send_event(&mut self, event: &WindowEvent) {
        match event {
            WindowEvent::CursorMoved { position, .. } => {
                ...
                self.proxy.pointer_move(wasi_surface_wasmtime::PointerEvent {
                    x: position.x,
                    y: position.y,
                });
            ...
            WindowEvent::KeyboardInput { event: input, .. } => {
                ...
                match input.state {
                    ElementState::Pressed => {
                        self.proxy.key_down(event);
                    }
                    ElementState::Released => {
                        self.proxy.key_up(event);
                    }
                }
            }
            WindowEvent::MouseInput { state, .. } => {
                let (pointer_x, pointer_y) = self.pointer_pos;
                ...
                match state {
                    ElementState::Pressed => {
                        self.proxy.pointer_down(event);
                    }
                    ElementState::Released => {
                        self.proxy.pointer_up(event);
                    }
                }
            }
            WindowEvent::Resized(new_size) => {
                self.proxy.canvas_resize(wasi_surface_wasmtime::ResizeEvent {
...
pub struct MyWindowWrapper(pub Arc<Window>);
impl HasDisplayHandle for MyWindowWrapper {
    fn display_handle(&self) -> Result<DisplayHandle<'_>, HandleError> {
        self.0.display_handle()
    }
}
impl HasWindowHandle for MyWindowWrapper {
    fn window_handle(&self) -> Result<WindowHandle<'_>, HandleError> {
        self.0.window_handle()
    }
}
impl DisplayApi for MyWindowWrapper {
    fn height(&self) -> u32 {
...
// impl TryFrom<winit::keyboard::KeyCode> for wasi_surface_wasmtime::wasi::surface::surface::Key {
//     type Error = ();

//     fn try_from(value: winit::keyboard::KeyCode) -> Result<Self, Self::Error> {
//         use wasi_surface_wasmtime::wasi::surface::surface::Key;
//         match value {
